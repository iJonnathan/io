<template>
  <div class=" contactme" id="contactme" >
    ...
    <div class="section-contact">
      ...
      <div :class="{ 'divGifSendMessage': isSendingMessage }">
...
export default {
  ...
  mounted(){
    ...
    if(process.env.NODE_ENV == 'production'){
      axios.get(this.serverIp+'/init').then(response => {
...
.divGifSendMessage{
  height: 100%;
  width:100%;
  background-image: url("../assets/loadSendMassage.gif");
  background-repeat: no-repeat;
  background-size: auto;
  background-position: center;
  background-color: rgba(71, 77, 82, 0.2);
  text-align: center;
